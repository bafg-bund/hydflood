...
if (!(file.exists("data/spdf.tiles_rhein.rda"))) {
    ...
    for (i in 1:length(spdf.tiles_rhein)) {
        spdf.tiles_rhein$xmin[i] <- as.integer(floor(min(spdf.tiles_rhein@polygons[[i]]@Polygons[[1]]@coords[,1])))
        spdf.tiles_rhein$xmax[i] <- as.integer(ceiling(max(spdf.tiles_rhein@polygons[[i]]@Polygons[[1]]@coords[,1])))
        spdf.tiles_rhein$ymin[i] <- as.integer(floor(min(spdf.tiles_rhein@polygons[[i]]@Polygons[[1]]@coords[,2])))
        spdf.tiles_rhein$ymax[i] <- as.integer(ceiling(max(spdf.tiles_rhein@polygons[[i]]@Polygons[[1]]@coords[,2])))
