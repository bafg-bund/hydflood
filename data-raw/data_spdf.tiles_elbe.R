...
if (!(file.exists("data/spdf.tiles_elbe.rda"))) {
    ...
    for (i in 1:length(spdf.tiles_elbe)) {
        spdf.tiles_elbe$xmin[i] <- as.integer(floor(min(spdf.tiles_elbe@polygons[[i]]@Polygons[[1]]@coords[,1])))
        spdf.tiles_elbe$xmax[i] <- as.integer(ceiling(max(spdf.tiles_elbe@polygons[[i]]@Polygons[[1]]@coords[,1])))
        spdf.tiles_elbe$ymin[i] <- as.integer(floor(min(spdf.tiles_elbe@polygons[[i]]@Polygons[[1]]@coords[,2])))
        spdf.tiles_elbe$ymax[i] <- as.integer(ceiling(max(spdf.tiles_elbe@polygons[[i]]@Polygons[[1]]@coords[,2])))
